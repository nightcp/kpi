...
export const departmentApi = {
  ...
  create: (data: Omit<Department, 'id' | 'created_at'>): Promise<{ data: Department }> => api.post('/departments', data),
...
export const employeeApi = {
  ...
  getSubordinates: (id: number): Promise<{ data: Employee[]; total: number }> => api.get(`/employees/${id}/subordinates`),
};
...
export const templateApi = {
  ...
  create: (data: Omit<KPITemplate, 'id' | 'created_at'>): Promise<{ data: KPITemplate }> => api.post('/templates', data),
...
export const evaluationApi = {
  ...
  create: (data: Omit<KPIEvaluation, 'id' | 'created_at'>): Promise<{ data: KPIEvaluation }> => api.post('/evaluations', data),
  update: (id: number, data: Partial<KPIEvaluation>): Promise<{ data: KPIEvaluation }> => api.put(`/evaluations/${id}`, data),
  delete: (id: number): Promise<void> => api.delete(`/evaluations/${id}`),
  getByEmployee: (employeeId: number): Promise<{ data: KPIEvaluation[]; total: number }> => api.get(`/evaluations/employee/${employeeId}`),
  getPending: (employeeId: number): Promise<{ data: KPIEvaluation[]; total: number }> => api.get(`/evaluations/pending/${employeeId}`),
};

// KPI评分API
export const scoreApi = {
  getByEvaluation: (evaluationId: number): Promise<{ data: KPIScore[]; total: number }> => api.get(`/scores/evaluation/${evaluationId}`),
  updateSelf: (id: number, data: { self_score?: number; self_comment: string }): Promise<{ data: KPIScore }> => api.put(`/scores/${id}/self`, data),
  updateManager: (id: number, data: { manager_score?: number; manager_comment: string }): Promise<{ data: KPIScore }> => api.put(`/scores/${id}/manager`, data),
  updateFinal: (id: number, data: { final_score?: number }): Promise<{ data: KPIScore }> => api.put(`/scores/${id}/final`, data),
};
...
export const exportApi = {
  ...
  period: (period: string): Promise<ExportResponse> => api.get(`/export/period/${period}`),
};
